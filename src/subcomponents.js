...
export function addNewFormBtns(parent) {
  ...
  const newProject = addElement("button", newFormBtnsDiv, [], null, {
    id: "new-project",
  });
  addImage(Plus, newProject);
  addElement("p", newProject, [], "New Project");

  const newTask = addElement("button", newFormBtnsDiv, [], null, {
    id: "new-task",
  });
...
export function createTaskItem(
  ...
  const itemDiv = addElement(
    "div",
    parent,
    ["item", task.getPriority()],
    null,
    { id: generateTaskItemId(task.getId()) }
  );
  ...
  if (task.getProjectId() > 0 && onTasksPage) {
    addElement(
      ...
      labelContent,
      ["project-title"],
      todos.getProjectById(task.getProjectId()).getTitle()
    );
  }
  ...
  const edit = addElement("button", wrapper, ["edit-button"], null, {
