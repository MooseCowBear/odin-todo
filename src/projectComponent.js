...
export function projectComponent(todos, projectId, parent) {
  ...
  addElement("button", header, ["edit-project"], "Edit", {
    id: "edit-project",
  });
  ...
  for (const cat of getCategories(tasks)) {
    ...
    const sortedTasks = orderByDate(
      tasks.filter((elem) => elem.getCategory() === cat)
    );
...
function addButtonListeners(parent, todos, projectId) {
  ...
  projectEditBtn.addEventListener("click", () => {
    projectFormComponent(
      ...
      formDiv,
      todos,
      todos.getProjectById(projectId)
    );
  });
  ...
  for (let i = 0; i < editBtns.length; i++) {
    editBtns[i].addEventListener("click", (e) => {
      const taskDiv = document.getElementById(
